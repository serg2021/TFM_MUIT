...
class CRO_SL:
    ...
    def save_data(self, solution_file=None, population_file=None, history_file=None, prob_file=None, indiv_history=None):
        ...
        if isinstance(ind, list):   #PARA TIPOS DE RECURSOS
            ind_aux = list(flatten(ind))
            ind_aux2 = np.array(ind_aux)
            np.savetxt(solution_file, ind_aux2.reshape([1, -1]), delimiter=',')
        else:
            np.savetxt(solution_file, ind.reshape([1, -1]), delimiter=',')
        
        pop_matrix = [i.solution for i in self.population.population]

        if isinstance(pop_matrix[0], list):         #PARA TIPOS DE RECURSOS
            lista_flatten = list(flatten(pop_matrix))
