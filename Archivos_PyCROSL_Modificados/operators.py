    ...
    if isinstance(vector[0], list):
        ...
        for i in mask_pos_num:
            ...
            if isinstance(vector[0][i], list):
                ...
                val = random.randint(0,numero_supply_depots-1)
                for k in ind_val:
                    if int(rand_vec[ind_vec]) == numero_supply_depots:
                        vector[1][i][k] = val
                    else:
            ...
            if isinstance(solution1[0][i], list) or isinstance(solution2[0][i], list):
                if isinstance(solution1[0][i], list):
                    for j in range(len(solution1[0][i])):
                        ...
                        val = random.randint(0, numero_supply_depots-1)
                        for l in ind_val:
                            if round(alpha * solution1[1][i][l]) == numero_supply_depots or round(alpha * solution1[0][i][j]) == numero_supply_depots:
                                solution1[1][i][l] = val
                                solution1[0][i][j] = val
                            else:
                    ...
                    for j in range(len(solution2[0][i])):
                        ...
                        val = random.randint(0, numero_supply_depots-1)
                        for l in ind_val:
                            if round((1-alpha) * solution2[1][i][l]) == numero_supply_depots or round((1-alpha) * solution2[0][i][j]) == numero_supply_depots:
                                solution2[1][i][l] = val
                                solution2[0][i][j] = val
                            else:
                    ...
                    val = random.randint(0, numero_supply_depots-1)
                    for l in ind_val_1:
                        if round(alpha * solution1[1][i][l]) == numero_supply_depots:
                            solution1[1][i][l] = val
                        else:
                            solution1[1][i][l] = round(alpha * solution1[1][i][l])
                    if round(alpha * solution1[0][i]) == numero_supply_depots:
                        solution1[0][i] = val
                    else:
                    ...
                    val = random.randint(0, numero_supply_depots-1)
                    for l in ind_val_2:
                        if round((1 - alpha) * solution2[1][i][l]) == numero_supply_depots:
                            solution2[1][i][l] = val
                        else:
                            solution2[1][i][l] = round((1 - alpha) * solution2[1][i][l])
                    if round((1 - alpha) * solution2[0][i]) == numero_supply_depots:
                        solution2[0][i] = val
                    else:
    ...
    #Una vez aplicado BLX tanto a las soluciones como a las asignaciones de clases a SD, las sumamos y obtenemos el coral definitivo
        ...
        for i in range(len(solution1[0])):
            if isinstance(solution1[0][i], list) and isinstance(solution2[0][i], list): #Sumo 2 listas
                ind_val_1 = [j for j,v in enumerate(solution1[1][i]) if v != numero_supply_depots]    #Sacamos las clases en cada base
                ind_val_2 = [j for j,v in enumerate(solution2[1][i]) if v != numero_supply_depots]    #Sacamos las clases en cada base
                contador = []
                for j in ind_val_1:
                ...
                if len(contador) > 0:
                    ...
                    for j in vals:
                        ...
                        res = val_1 + val_2
                        if res == numero_supply_depots:
                            res = random.randint(0,numero_supply_depots-1)
                        solution3[1][i][j] = res

                        #Ya tenemos actualizadas las clases. Ahora actualizamos la solución

                    solution3[0][i] = []
                    val_3 = [v for j, v in enumerate(solution3[1][i]) if v != numero_supply_depots]  # Sacamos las clases en cada base
                    for j in val_3:
        ...
        if len(population) > 3:
                ...
                if isinstance(r1.solution[0][i], list) or isinstance(r2.solution[0][i], list):
                    if isinstance(r1.solution[0][i], list):
                        for j in range(len(r1.solution[0][i])):
                            ...
                            val = random.randint(0,numero_supply_depots-1)
                            for l in ind_val:
                                if round(F * r1.solution[1][i][l]) == numero_supply_depots:
                                    r1.solution[1][i][l] = val
                                else:
                                    r1.solution[1][i][l] = round(F * r1.solution[1][i][l])
                            if round(F * r1.solution[0][i][j]) == numero_supply_depots:
                        ...
                        for j in range(len(r2.solution[0][i])):
                            ...
                            val = random.randint(0, numero_supply_depots-1)
                            for l in ind_val:
                                if round(F * r2.solution[1][i][l]) == numero_supply_depots:
                                    r2.solution[1][i][l] = val
                                else:
                                    r2.solution[1][i][l] = round(F * r2.solution[1][i][l])
                            if round(F * r2.solution[0][i][j]) == numero_supply_depots:
                        ...
                        for l in ind_val:
                            if round(F * r1.solution[1][i][l]) == numero_supply_depots:
                                ...
                                r1.solution[1][i][l] = round(F * r1.solution[1][i][l])
                        if round(F * r1.solution[0][i]) == numero_supply_depots:
                            r1.solution[0][i] = val
                        else:
                        ...
                        for l in ind_val_2:
                            if round(F * r2.solution[1][i][l]) == numero_supply_depots:
                                ...
                                r2.solution[1][i][l] = round(F * r2.solution[1][i][l])
                        if round(F * r2.solution[0][i]) == numero_supply_depots:
                            r2.solution[0][i] = val
                        else:
            ...
            for i in range(len(r1.solution[0])):
                if isinstance(r1.solution[0][i], list) and isinstance(r2.solution[0][i], list):  # Sumo 2 listas
                    ind_val_1 = [j for j, v in enumerate(r1.solution[1][i]) if v != numero_supply_depots]  # Sacamos las clases en cada base
                    ind_val_2 = [j for j, v in enumerate(r2.solution[1][i]) if v != numero_supply_depots]  # Sacamos las clases en cada base
                    contador = []
                    for j in ind_val_1:
                        if j in ind_val_2:  # Comprobamos que comparten la misma clase para poder sumar
                            contador.append(j)
                    if len(contador) > 0:
                        ...
                        solution_aux[0][i] = []
                        val_3 = [v for j, v in enumerate(solution_aux[1][i]) if v != numero_supply_depots]  # Sacamos las clases en cada base
                        for j in val_3:
            ...
            for i in range(len(best.solution[0])):
                if isinstance(best.solution[0][i], list) and isinstance(solution_aux[0][i], list):  # Sumo 2 listas
                    ind_val_1 = [j for j, v in enumerate(best.solution[1][i]) if v != numero_supply_depots]  # Sacamos las clases en cada base
                    ind_val_2 = [j for j, v in enumerate(solution_aux[1][i]) if v != numero_supply_depots]  # Sacamos las clases en cada base
                    contador = []
                    for j in ind_val_1:
                        if j in ind_val_2:  # Comprobamos que comparten la misma clase para poder sumar
                            contador.append(j)
                    if len(contador) > 0:
                        ...
                        val[0][i] = []
                        val_3 = [v for j, v in enumerate(val[1][i]) if v != numero_supply_depots]  # Sacamos las clases en cada base
                        for j in val_3:
                            val[0][i].append(j)
