        ...
        for i in range(self.Num_Individuos - self.Num_Padres): #Bucle para generar HIJOS
            ...
            if any(vector):
                for j in enumerate(np.where(vector==1)[0]):
                    ...
                    Asig_Hijo[j[1]] = lista[Indice_Padres[1]][j[1]]  #Vamos actualizando la lista de asignaciones para que sea la misma
    ...
    def Comprobacion_Individuo (self, individuo, asig, capacidades):
        ...
        for i in range(self.Num_Max):   #Bucle para cada SD
            ...
            for j in range(len(indices_bases)): #Bucle para base asociada a ese SD
                ...
                #valor_clases = [v for k, v in enumerate(Asig_Clases_SD[indices_bases[j]]) if v != numero_supply_depots]  # Sacamos qué clases tiene esa base
                #valor_clases_indi = [v for k, v in enumerate(individuo[indices_bases_list[j]]) if v != numero_supply_depots]  # Sacamos qué clases tiene esa base
                ...
                while True: #Bucle para solucionar los SD
                    ...
                    while True: #Bucle infinito hasta que no dé errores para UNA CLASE DEL SD
                        while True:
                            ...
                            lista_filtrada = [v for v in indices_resto_bases if capacidades[v][k] <= capacidades[indice_base_1][k] and capacidades[v][k] != 0]
                            if lista_filtrada:
                        ...
                        if isinstance(individuo[indice_base_1], (int, np.integer)) and isinstance(individuo[indice_base_aleatoria_2], (int, np.integer)):
                            ...
                            if len(comprob_asig_1) == 1 and len(comprob_asig_2) == 1:   #Si sólo tienen 1 clase las 2 bases... Las podemos intercambiar
                                ...
                                if len(comprob_asig_1) > 1:
                                    asig[indice_base_1][k] = individuo[indice_base_aleatoria_2]  #Actualizamos la asignación de esa clase al nuevo SD
                                    ...
                                    if k in comprob_asig_2:
                                        if len(comprob_asig_2) > 1:
                                            asig[indice_base_aleatoria_2][k] = individuo[indice_base_1][0]  # Actualizamos la asignación de esa clase al viejo SD
                                            individuo[indice_base_aleatoria_2] = [individuo[indice_base_aleatoria_2]]  # Lo pasamos a lista
                                            individuo[indice_base_aleatoria_2].append(individuo[indice_base_1][0])  # Le añadimos el viejo SD
                                        elif len(comprob_asig_2) == 1:
                                            asig[indice_base_aleatoria_2][k] = individuo[indice_base_1][0]  # Actualizamos la asignación de esa clase al nuevo SD
                                            individuo[indice_base_aleatoria_2] = individuo[indice_base_1][0]
                                    ...
                                    if k in comprob_asig_2:
                                        if len(comprob_asig_2) > 1:
                                            asig[indice_base_aleatoria_2][k] = SD_aux # Actualizamos la asignación de esa clase al viejo SD
                                            individuo[indice_base_aleatoria_2] = [individuo[indice_base_aleatoria_2]]  # Lo pasamos a lista
                                            individuo[indice_base_aleatoria_2].append(SD_aux)  # Le añadimos el viejo SD
                                        elif len(comprob_asig_2) == 1:
                                            asig[indice_base_aleatoria_2][k] = SD_aux # Actualizamos la asignación de esa clase al nuevo SD
                            ...
                            asig[indice_base_1][k] = k_3  # Actualizamos la asignación de esa clase al nuevo SD
                            ...
                            asig[indice_base_aleatoria_2][k] = SD_ind  # Actualizamos la asignación de esa clase al nuevo SD
                            ...
                            if isinstance(individuo[indice_base_1], list):
                                asig[indice_base_1][k] = individuo[indice_base_aleatoria_2]
                                comprob_asig_1 = [i for i, v in enumerate(asig[indice_base_1]) if v == SD_ind]
                                if len(comprob_asig_1) == 0:
                                    ind_cambio = individuo[indice_base_1].index(SD_ind)
                                    individuo[indice_base_1].pop(ind_cambio)  # Le quitamos ese SD
                                ...
                                asig[indice_base_aleatoria_2][k] = SD_ind
                                ...
                                asig[indice_base_aleatoria_2][k] = individuo[indice_base_1]
                                comprob_asig_2 = [i for i, v in enumerate(asig[indice_base_aleatoria_2]) if v == k_3]
                                if len(comprob_asig_2) == 0:
                                    ind_cambio = individuo[indice_base_aleatoria_2].index(k_3)
                                    individuo[indice_base_aleatoria_2].pop(ind_cambio)  # Le quitamos ese SD
